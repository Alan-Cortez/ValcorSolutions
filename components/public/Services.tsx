          ...
          >
            {capabilities.map((service, i) => {
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.4rem', paddingTop: '2px', minWidth: 28 }}>
                    <span style={{ fontSize: '0.58rem', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.1em' }}>
                      {String(i + 1).padStart(2, '0')}
                    </span>
                    <Icon size={15} color="var(--text-secondary)" strokeWidth={1.5} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <h3 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '0.3rem', letterSpacing: '-0.01em', color: 'var(--white)' }}>
                      {service.title}
                    </h3>
            ...
            <div style={{
              ...
              {[
                ...
                <div key={item.label} style={{ marginBottom: '1.25rem' }}>
                  ...
                  <div style={{ height: '2px', background: 'var(--bg-border)', borderRadius: '1px', overflow: 'hidden' }}>
                    <motion.div
                      initial={{ scaleX: 0 }}
                      whileInView={{ scaleX: 1 }}
                      viewport={{ once: true }}
                      transition={{ duration: 1, delay: 0.3, ease: [0.16, 1, 0.3, 1] }}
                      style={{ height: '100%', width: `${item.pct}%`, background: 'var(--white)', transformOrigin: 'left' }}
                    />
                  </div>
                </div>
              ))}
